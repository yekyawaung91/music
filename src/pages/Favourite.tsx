...
import { Grid, List, Play, Pause, Eye, Heart } from "lucide-react";
...
export default function Favourite() {
  ...
  // Filter favourite songs
  const favourites: Song[] = songsData.filter(song => song.favourite);
  ...
  <div
        className={`flex items-center justify-center w-6 h-6 rounded-full transition-colors ${
          isFavourite ? "bg-pink-500" : "bg-white/10 hover:bg-pink-500"
        }`}
        onClick={(e) => {
          e.stopPropagation();
      ...
      >
        <Heart
          size={16}
          className={`transition-colors ${isFavourite ? "text-white" : "text-pink-400 hover:text-white"}`}
        />
      </div>
